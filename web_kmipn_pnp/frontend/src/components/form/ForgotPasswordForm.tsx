                ...
                <div className="form-group mb-4">
                    <div className="form-field">
                        ...
                        <label className="form-label">
                            <Link href={"/auth/login"} className="link link-underline-hover link-primary text-sm">Sudah ingat? Login</Link>
                        </label>
                    </div>
                </div>
                <button type="submit" disabled={isLoading} className={`btn btn-md btn-success rounded-lg min-w-full ${isLoading && "btn-loading"}`}>{isLoading ? "Loading..." : "Kirim Link Reset"}</button>
            </form>
        </>
    );
}